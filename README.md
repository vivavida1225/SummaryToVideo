...
문장 수, 인사말 위치, 650자 상한 및 첫 줄의 지수별 종가·등락률·방향을 검사합니다. 천 단위 쉼표 차이, `%`와 ‘퍼센트’ 표기, `0.00%`의 ‘보합’ 표현은 허용합니다. 400~550자는 권장 기준입니다. 글자 수에는 인사말·숫자·공백·문장부호를 포함하고 개행만 제외합니다. 검증은 서술 전체의 사실성·인과관계를 보증하지 않으며, 내용의 충실성과 약 1분의 실제 낭독 시간은 최종 편집 단계에서 확인하세요.

새 출력 계약은 새로 생성하는 결과에 적용합니다. 기존 저장 결과는 변환 없이 다시 열고 복사·다운로드할 수 있습니다. 백엔드 코드 변경 후 실행 중인 앱은 종료하고 다시 실행하세요.
